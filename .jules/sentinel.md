...
## 2025-01-21 - Argument Injection in Safe Command Execution
**Vulnerability:** The `safe_command` tool whitelisted command executables (e.g., `dig`, `ping`) but did not validate their arguments, allowing attackers to use flags like `dig -f` to read arbitrary files or `ping -f` for DoS.
**Learning:** Whitelisting executables is insufficient for security; many standard utilities have dangerous flags that transform their behavior (file reading, execution, etc.).
**Prevention:** Implement argument validation alongside command whitelisting. Use a strict blocklist for known dangerous flags or, preferably, a strict allowlist of permitted flags and argument patterns for each tool.
